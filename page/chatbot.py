...
with st.sidebar:
    ...
    with tab1:
        ...
        citation_radio = st.radio("Include CV Citations", ["Yes", "No"],
        help="Enable to include CV citation at the bottom of chatbot's response.")
    ...
    with tab2:
        ...
        ## Style Guide
        Start with '####' for headers. Use British spelling, and maintain a professional tone. 
        ''',
...
if prompt := st.chat_input("Ask a question about the uploaded CVs"):
    ...
    with st.chat_message("assistant"):
        ...
        with status_placeholder.status("Thinking...") as status:
            ...
            full_response = ''          # Full response to be displayed in the chat
            full_documents = []         # command model citation output
            intext_citations = []       # command model citation output
            cited_documents = []        # List of dictionaries to store the cited documents
            for event in response:
                if event.event_type == "text-generation":
                    ...
                    intext_citations.extend(event.citations)  
                elif event.event_type == "search-results":
                    full_documents = event.documents
            if citation_radio == "Yes":         # Include CV citations at the bottom of the response
                full_response+= "---"
                full_response+= "\n\n #### Relevant CVs:"
                for citation in intext_citations:
                    for doc_id in citation.document_ids:        
                        # Find the cited document in the list of dictionaries and add it to the cited_documents list
                        document = next((doc for doc in full_documents if doc['id'] == doc_id), None)
                        if document and not any(doc['filename'] == document['filename'] for doc in cited_documents):
                            cited_documents.append({'filename': document['filename'], 'team': document['team'], 'link': document['link']})
                for i in range(len(cited_documents)):
                    full_response += "\n"
                    full_response += " - "
                    full_response += f"[{i+1}] [:violet[{cited_documents[i]['filename']}]]({cited_documents[i]['link']}) [{cited_documents[i]['team']}]"
                # full_response += "]"
                text_placeholder.markdown(full_response)
